...
class NLIModel:
    def __init__(self, model_name: str = "microsoft/deberta-large-mnli"):
        ...
        print(f"NLI model loaded on {self.device}: {model_name}")

    @torch.no_grad()
    def entail_prob_batch(self, prem_list: list, hyp_list: list, batch_size: int = 8) -> np.ndarray:
        ...
        for i in range(0, len(prem_list), batch_size):
            ...
            enc = self.tokenizer(prem, hyp, return_tensors="pt", truncation=True, padding=True, max_length=384).to(self.device)
            logits = self.model(**enc).logits.float()
            probs = torch.softmax(logits, dim=-1)[:, 2]  # entailment index
            out.append(probs.detach().cpu().numpy())
        return np.concatenate(out, axis=0)

    def pairwise_entailment_matrix(self, samples: list) -> np.ndarray:
        n = len(samples)
        P = np.zeros((n, n), dtype=np.float32)
        for i in range(n):
            prem = [samples[i]] * n
            hyp  = list(samples)
            P[i,:] = self.entail_prob_batch(prem, hyp)
        return P
    ...
    def shapley_uncertainty_for_prompt(self, prompt: str, gold: str | None = None, n_samples: int = 20, M: int = 1000,
        ...
        q = None
        if gold:
            ent1 = self.entail_prob_batch([gold]*len(samples), samples)
            ent2 = self.entail_prob_batch(samples, [gold]*len(samples))
            q = (ent1 + ent2)/2.0
        def v(S):
